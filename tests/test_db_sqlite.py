...
from db.db_sqlite import SQLiteDatabase # Import the class
...
def test_connect_db_success(api_client, app, db_instance):
...
def test_create_tables_executes_correct_sql(api_client, app, db_instance):
    """
...
def test_insert_user_function(api_client, app, db_instance):
...
def test_insert_duplicate_user(api_client, app, db_instance):
...
def test_find_by_username(api_client, app, db_instance):
...
def test_add_event_function(api_client, app, db_instance):
...
def test_get_events_no_filter(api_client, app, db_instance):
    """Test retrieving all events without filters."""
    event1 = {"timestamp": datetime.now(timezone.utc).isoformat(), "hostname": "h1", "event_type": "e1", "message": "m1"}
    event2 = {"timestamp": (datetime.now(timezone.utc) - timedelta(hours=1)).isoformat(), "hostname": "h2", "event_type": "e2", "message": "m2"}
...
def test_get_events_with_date_filter(api_client, app, db_instance):
...
def test_get_all_events(api_client, app, db_instance):
...
def test_get_alerts_placeholder(api_client, app, db_instance):
