...
from tests.conftest import mock_mongo_db_instance, app, client # Import necessary fixtures
...
def test_mongo_get_all_events(mock_mongo_db_instance):
    ...
    mock_all_events_from_db = [
        {"_id": "60c72b2f9b1d8e001f8e4d6d", "timestamp": datetime.now(timezone.utc), "hostname": "hostC", "event_type": "typeZ", "message": "msg3", "details": None, "ip": "3.3.3.3", "process": "proc3"},
        {"_id": "60c72b2f9b1d8e001f8e4d6e", "timestamp": datetime.now(timezone.utc), "hostname": "hostD", "event_type": "typeW", "message": "msg4", "details": json.dumps({"d2":2}), "ip": None, "process": None},
    ]
...
def test_mongo_get_alerts(mock_mongo_db_instance):
    ...
    mock_alerts_from_db = [
        {"_id": "60c72b2f9b1d8e001f8e4d6f", "alert_type": "brute_force", "timestamp": datetime.now(timezone.utc)},
        {"_id": "60c72b2f9b1d8e001f8e4d70", "alert_type": "anomaly", "timestamp": datetime.now(timezone.utc)},
    ]
    mock_find_cursor = MagicMock()
