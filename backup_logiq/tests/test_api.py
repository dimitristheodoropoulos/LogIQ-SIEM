...
def test_get_alerts_success(client, app):
    ...
    mock_events = [
        {"timestamp": (datetime.now(timezone.utc) - timedelta(hours=1)).isoformat() + "Z", "hostname": "h1", "event_type": "login_fail", "message": "fail", "ip": "1.1.1.1"},
        {"timestamp": datetime.now(timezone.utc).isoformat() + "Z", "hostname": "h1", "event_type": "login_fail", "message": "fail", "ip": "1.1.1.1"}
    ]
...
def test_generate_report_success(client, app):
    ...
    mock_events = [
        {"timestamp": datetime.now(timezone.utc).isoformat() + "Z", "hostname": "h1", "event_type": "login_success", "message": "msg1"},
        {"timestamp": (datetime.now(timezone.utc) - timedelta(hours=1)).isoformat() + "Z", "hostname": "h2", "event_type": "login_fail", "message": "msg2"}
    ]
...
def test_export_json_success(client, app, tmp_path):
    ...
    # Patch the export_logs_function from api.routes
    temp_file_path = tmp_path / "temp_export.json"
    with patch('api.routes.export_logs_function', return_value=str(temp_file_path)) as mock_export_logs:
