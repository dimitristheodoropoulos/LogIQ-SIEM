...
def test_report_export_json_success(client, mock_sqlite_db_instance, mock_report_generator, app, tmp_path):
    ...
    # Patch the internal helper function _export_report_summary in api.routes
    with patch('api.routes._export_report_summary', return_value=str(temp_report_file)) as mock_export_report_summary:
