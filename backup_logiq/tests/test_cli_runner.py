...
from cli.runner import run_parse_logs, run_alerts, run_report, run_db_connection_test, run_cli_command
from db.db_sqlite import SQLiteDatabase
from db.db_mongo import MongoDB # Import MongoDB for mocking
from reports.report_generator import ReportGenerator # Import for type hinting and patching
from parsers.auth_parser import parse_auth_log # Import for type hinting and patching
...
def mock_app_context(mock_db_instance, mock_mongo_db_instance, mock_config):
    ...
    with patch('main.load_config', return_value=mock_config):
        ...
        with patch('main.SQLiteDatabase', return_value=mock_db_instance), \
             patch('main.MongoDB', return_value=mock_mongo_db_instance):
            ...
            # Patch create_app in main to return our mock_app
...
def test_run_parse_logs_success(mock_app_context, mock_db_instance):
    ...
    mock_events = [
        {"timestamp": datetime.now(timezone.utc).isoformat(), "hostname": "test", "event_type": "login_success", "message": "msg1"},
        {"timestamp": datetime.now(timezone.utc).isoformat(), "hostname": "test", "event_type": "login_fail", "message": "msg2"}
    ]
    # Patch the function where it's *imported and used* within cli.runner
    with patch('cli.runner.parse_auth_log', return_value=mock_events) as mock_parse_auth_log:
...
def test_run_parse_logs_no_events(mock_app_context, mock_db_instance):
    ...
    with patch('cli.runner.parse_auth_log', return_value=[]) as mock_parse_auth_log:
        with patch('sys.stdout', new_callable=StringIO) as captured_output:
            run_parse_logs(mock_app_context)
            mock_parse_auth_log.assert_called_once_with(mock_app_context.config.get('LOG_FILE_PATH'))
...
def test_run_alerts_with_alerts(mock_app_context):
    ...
    mock_app_context.db.get_all_events.return_value = [
        {"timestamp": datetime.now(timezone.utc).isoformat(), "event_type": "Failed login", "username": "baduser", "ip": "1.1.1.1", "message": "test"}
    ]
...
def test_run_report_success(mock_app_context):
    ...
    mock_events_for_report = [
        {"timestamp": datetime.now(timezone.utc).isoformat(), "event_type": "login_success", "message": "msg1"}
    ]
...
def test_run_cli_command_parse_logs(mock_app_context):
    ...
    with patch('cli.runner.run_parse_logs') as mock_run_parse_logs:
...
def test_run_cli_command_alerts(mock_app_context):
    ...
    with patch('cli.runner.run_alerts') as mock_run_alerts:
...
def test_run_cli_command_report_success(mock_app_context):
    ...
    with patch('cli.runner.run_report') as mock_run_report:
...
def test_run_cli_command_db_test(mock_app_context):
    ...
    with patch('cli.runner.run_db_connection_test') as mock_run_db_test:
